...
def get_repos(username, token):
    ...
    token should be a tuple of username and password.
...
def get_comments(username, repo, number, token):
...
def get_pulls(username, repo, token):
    """ username and repo should be strings
    token should be a tuple of username and password.
    """
...
def _getter(url, token):
    """ Pagination utility.  Obnoxious. """

    results = []
    link = dict(next=url)
    while 'next' in link:
        response = requests.get(link['next'], params=dict(access_token=token))
